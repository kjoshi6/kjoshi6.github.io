// ==== Initialize Swiss Ephemeris for browser ====
let swe;

// Wait until the page and swe library are loaded
window.addEventListener("load", async () => {
  console.log("Initializing Swiss Ephemeris...");
  try {
    if (!window.swe) {
      throw new Error("Swiss Ephemeris not found in window.");
    }
    swe = window.swe;
...
// ==== Form submission and chart calculation ====
document.getElementById('birth-form').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (!selectedLocation) {
      output.textContent = 'Start typing a city...';
      return;
    }
